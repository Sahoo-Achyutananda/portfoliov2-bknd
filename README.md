...
cp .env.example .env       # then fill in GOOGLE_API_KEY when needed
...
Plan for replacing the dummy `/chat` response with a real LangGraph + Gemini agent that can
...
1. **Install the agent stack**
   ...
   ```
   Already added to `requirements.txt`, so `pip install -r requirements.txt` picks them up too.

2. **Get a Gemini key and load it at startup**
   Grab a free key from Google AI Studio (aistudio.google.com) — no billing required to start,
   unlike the Anthropic console. Put it in `.env` as `GOOGLE_API_KEY=...` (already the placeholder
   in `.env.example`). `python-dotenv` is already a dependency but nothing calls it yet — add
...
3. **Mirror the portfolio data as Python tools**
   Already done — `data.py` has `PROJECTS`, `EXPERIENCE`, `CERTIFICATES` plus `projects_to_cards()`
   / `experience_to_cards()` / `certificates_to_cards()`, currently used by the dummy `/chat`
   keyword matcher. Wrap each converter as a LangChain tool with `@tool`, e.g. `get_projects()`,
   `get_experience()`, `get_certificates()`.
...
4. **Turn the existing stats fetchers into tools too**
   ...
   core logic into plain async functions (in `data.py` or a new `tools.py`, not FastAPI route
   handlers) and wrap *those* with `@tool` as `get_leetcode_stats()` / `get_github_stats()`. The
   existing `/leetcode/stats` and `/github/stats` routes can just call the same underlying
   functions, so there's one source of truth instead of the agent hitting its own HTTP API.
...
5. **Build the first agent with the prebuilt ReAct constructor**
   ...
   `ChatGoogleGenerativeAI(model="gemini-...")` and the tool list from steps 3–4. This is already
   a real LangGraph `StateGraph` under the hood — it's the standard on-ramp before writing custom
   nodes.
...
7. **Wire the agent into `POST /chat`**
   Replace the keyword-matching logic with a call into the agent. Keep the existing
   `ChatRequest`/`ChatResponse` shape (`reply` + optional `cards`) — have the agent's tools return
   the same card dicts `data.py` already produces, so the frontend's `ChatCardCarousel` keeps
   working unchanged.
